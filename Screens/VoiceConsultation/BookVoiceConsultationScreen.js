...
const BookVoiceConsultationScreen = ({ route }) => {
  ...
  const fetchTimeSlots = async selectedDate => {
    ...
    try {
      ...
      //console.log('Available slots:', formattedSlots);
       // ⚠️ Clear selected slot if not available anymore
    if (selectedTimeSlot && !formattedSlots.includes(selectedTimeSlot)) {
    ...
    try {
      ...
      const response = await axios.post(
        `${BaseUrl}/book-appointments`,
        {
          ...
          types: 3, // Video consultation type
        },
        {
          headers: {
        ...
        <View style={styles.profileInfo}>
          ...
          <View style={styles.statusBadge}>
           <FontAwesome name="microphone" size={14} color="#fff" />
            ...
            <Text style={styles.statusText}> Voice Consultations</Text>
          </View>
        </View>
      </View>
...
export default BookVoiceConsultationScreen;
